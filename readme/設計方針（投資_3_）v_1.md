...
- **実装状況 (2025-11-01)**
  - `scripts/run_daily_workflow.py --ingest` で `--use-dukascopy` / `--use-yfinance` を排他指定できる。Dukascopy 取得が失敗した際はオペレータが yfinance へ切り替え、`scripts/yfinance_fetch.py` が `period="7d"` で最新7日間を取得しつつ `start/end` でフィルタリングする（Yahoo Finance チャート API を直接呼び出すため追加ライブラリ不要）。
