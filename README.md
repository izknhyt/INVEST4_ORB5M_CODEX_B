...
- 重複タイムスタンプ内で OHLC/出来高/スプレッド値が一致しない場合は `duplicate_conflict_groups` / `duplicate_conflict_rows` / `duplicate_conflict_fields` / `duplicate_conflict_field_counts` で衝突の有無と影響列を把握できます。閾値未満で除外した重複でも衝突があれば `ignored_duplicate_conflict_groups` / `ignored_duplicate_conflict_rows` に記録されるため、フィルタの影響を後から追跡できます。
