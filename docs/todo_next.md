...
- **価格インジェストAPI基盤整備**（バックログ: `docs/task_backlog.md` → P1「ローリング検証 + 健全性モニタリング」） — `state.md` 2025-10-16, 2025-11-05, 2025-11-06 <!-- anchor: docs/task_backlog.md#p1-04-価格インジェストapi基盤整備 -->
  ...
  - 2025-11-19: yfinance フェイルオーバーの取得データが 90 分を超えて古い場合にローカル CSV + `synthetic_local` へ切り替える鮮度ガードを実装し、`ingest_meta.fallbacks` に stale 理由が残ることと snapshot 鮮度が 5 分以内へ戻ることを pytest で確認。
