...
- **価格インジェストAPI基盤整備**（バックログ: `docs/task_backlog.md` → P1「ローリング検証 + 健全性モニタリング」） — `state.md` 2025-10-16, 2025-11-05, 2025-11-06 <!-- anchor: docs/task_backlog.md#p1-04-価格インジェストapi基盤整備 -->
  ...
  - 2025-11-20: `ops/runtime_snapshot.json.ingest_meta.USDJPY_5m` を手動更新し、`dukascopy → yfinance → local_csv + synthetic_local` のフェイルオーバーチェーンを記録。`python3 scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6` の結果が `ok=true` + `advisories` となることを確認し、依存導入後に実データで再実行するタスクを継続。チェックリストと backlog メモを同期。
