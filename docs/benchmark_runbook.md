...
- Cron 後の鮮度確認として `python3 scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6 --benchmark-freshness-max-age-hours 6` を実行し、`benchmarks` および `benchmark_pipeline` のタイムスタンプが許容範囲に収まっているか検証する。`--benchmark-freshness-max-age-hours` は `benchmarks.*` の許容上限を個別に調整でき、未指定時は `--max-age-hours` の値が継承される。日次ワークフローからは `--check-benchmark-freshness` と `--benchmark-freshness-base-max-age-hours 6`（パイプライン閾値）および `--benchmark-freshness-max-age-hours 6` を併用して自動実行する。`ingest_metadata` ブロックには主経路（`primary_source`）、フェイルオーバー履歴（`fallbacks` / `source_chain`）、最新取得時刻（`last_ingest_at`）、鮮度差分（`freshness_minutes`）がまとめて表示されるため、合成バー (`synthetic_local`) やローカル CSV から復旧したケースでも経路を追跡しやすい。Sandbox では `source_chain` に `synthetic_local` が含まれる場合、`benchmark_pipeline.*` の欠損/遅延は `advisories` として報告されるため、実データ再取得後に `errors` が空へ戻ったか確認する。
...
1. Cron 実行後は `ops/runtime_snapshot.json` の `benchmarks.<symbol>_<mode>` および `benchmark_pipeline.<symbol>_<mode>` に最新タイムスタンプが追記され、`latest_ts` と `summary_generated_at` が同一時刻（パイプラインの生成完了時刻）で揃っているか確認する。`alert.payload.deltas` には今回の差分が保持される。
...
6. `scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6 --benchmark-freshness-max-age-hours 6` の出力を確認し、`ok` が `true` かつ `errors` が空であることをチェックする。`ingest_metadata.freshness_minutes` と `ingest_metadata.source_chain` を参照すると、前回のインジェスト経路と鮮度差分をワンショットで把握できる。`ok=false` の場合は JSON の `errors` を読み取り、`benchmarks`・`benchmark_pipeline.latest_ts`・`summary_generated_at` のいずれが閾値超過かを特定し、該当処理（ベンチマーク run / サマリー集計 / Cron 時刻）を再実行する。猶予を設ける必要がある場合は `--benchmark-freshness-max-age-hours`（benchmarks 側）を直接上書きするか、`run_daily_workflow.py --check-benchmark-freshness` を再実行する際に `--benchmark-freshness-base-max-age-hours` を調整し、理由を `state.md` と Cron 定義に記録する。
7. Fill 挙動の異常が疑われる場合は `python3 analysis/broker_fills_cli.py --format markdown` を実行し、OANDA / IG / SBI FXトレードの同足ヒット・トレール挙動と `core/fill_engine.py` Conservative / Bridge 出力を比較する。`docs/broker_oco_matrix.md` のポリシーと乖離があれば、`RunnerConfig.fill_same_bar_policy_*`（CLI: `--fill-same-bar-policy[-conservative|-bridge]`）や `fill_bridge_lambda` / `fill_bridge_drift_scale` を調整し、再度 `python3 -m pytest tests/test_fill_engine.py` を走らせる。
