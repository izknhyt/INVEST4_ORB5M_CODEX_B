...
- [x] Dukascopy フェッチ失敗/鮮度低下時に Yahoo Finance のチャート API へ自動フェイルオーバーする実装が `scripts/run_daily_workflow.py` に組み込まれ、回帰テストでカバーされている（追加パッケージ不要、`requests` のみで稼働）。
- [x] REST プロバイダ候補について `docs/api_ingest_plan.md#4-configuration` の `activation_criteria` を満たすか評価し、ターゲットコスト上限・無料枠・リトライ予算をチェックリストに記録した。
  - 2025-11-05 04:00Z 評価: `target_cost_ceiling_usd=40` / `minimum_free_quota_per_day=500` / `retry_budget_per_run=15` を基準に比較。
    ...
    - Yahoo Finance（0 USD, 約1req/分相当のチャート API, 7日分バッチ取得で60日履歴）→ 現行フェイルオーバー経路として継続、REST置換は不要。
...
- [x] `python3 scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6` が成功し、鮮度アラートが解消される（Dukascopy 主経路で代替中）。
  ...
  - 2025-11-13 04:10Z: Proxy 403 のため `pip install dukascopy-python` が失敗。`python3 scripts/run_daily_workflow.py --ingest --use-dukascopy --symbol USDJPY --mode conservative` はローカル CSV + `synthetic_local` で完走し、`ops/runtime_snapshot.json.ingest.USDJPY_5m` を 2025-10-02T03:15:00 まで更新したが、ベンチマーク側は 24.29h / 15.00h 遅延のまま `check_benchmark_freshness` が失敗。
  - 2025-11-27 14:50Z: `python3 scripts/run_daily_workflow.py --ingest --use-yfinance --symbol USDJPY --mode conservative`（HTTP フォールバック）と `python3 scripts/run_benchmark_pipeline.py --windows 365,180,90 --disable-plot` を実行後、`check_benchmark_freshness` が `ok: true` を返し、`benchmarks/benchmark_pipeline` の遅延は 0.07h 以内に収束。
