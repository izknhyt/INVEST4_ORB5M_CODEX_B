...
- 個別の実行例
  ...
  - Dukascopy 経由（標準経路）: `python3 -m scripts.run_daily_workflow --ingest --use-dukascopy --symbol USDJPY --mode conservative`
    - 失敗時や取得データが `--dukascopy-freshness-threshold-minutes`（既定 90 分）より古い場合は自動で yfinance へ切替。フォールバック時は `--yfinance-lookback-minutes`（既定 60 分）で再取得ウィンドウを決めつつ、Yahoo Finance チャート API を標準ライブラリで叩き 7 日ごとに自動分割しながら最大 60 日まで遡れる。長期停止後に再開する際は `--yfinance-lookback-minutes` を状況に応じて拡張してから実行する。Dukascopy 経路の事前準備として `pip install dukascopy-python` を推奨。yfinance フォールバックは標準ライブラリ実装に置き換えたため追加パッケージ不要。
    ...
    - 2025-11-10 02:15Z: yfinance パッケージ依存を除去し、Yahoo Finance チャート API を直接叩くフォールバックへ更新。`pip install dukascopy-python` のみで `--use-dukascopy` → yfinance 自動切替が動作するため、最小構成の環境でも ImportError が解消された。
  - API 直接取得（保留中）:
