...
- 2025-11-10: yfinance フォールバックを Yahoo Finance チャート API 直叩きの実装へ置き換え、追加パッケージ無しで `--use-dukascopy` → yfinance 自動切替が機能するようにした。`scripts/yfinance_fetch.py` を刷新し、`tests/test_yfinance_fetch.py` / README / state runbook / 設計書を同期。
- 2025-11-11: yfinance フォールバックで intraday 7 日ウィンドウを自動分割し、最大 60 日まで段階的に再取得できるよう改善。`scripts/yfinance_fetch.py` へウィンドウ生成ヘルパを追加し、`tests/test_yfinance_fetch.py::test_fetch_bars_chunks_intraday_requests` で連続フェッチと重複スキップを検証。README / state runbook / ingest plan / チェックリスト / todo_next を同期。
