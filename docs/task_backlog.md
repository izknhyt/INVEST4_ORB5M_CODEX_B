...
- 2025-11-21: `scripts/run_daily_workflow.py` に `--dukascopy-offer-side` を追加し、BID/ASK の選択を CLI から切替可能にした。選択サイドは `ingest_meta.primary_parameters.offer_side` に永続化され、`tests/test_run_daily_workflow.py::test_dukascopy_offer_side_metadata` で回帰を確保。README / state runbook へレビュー手順を追記。
