...
**進捗メモ**
- 2025-12-29: Adjusted `tests/test_fetch_prices_api.py` to stub `urllib.request.urlopen`, keeping retry/anomaly assertions while avoiding sandbox socket binds so pytest runs cleanly.
...
- 2025-11-28: `scripts/run_daily_workflow.py --check-benchmark-freshness` に `--benchmark-freshness-base-max-age-hours` を追加し、パイプライン閾値を CLI から明示指定できるようにした。README / docs/benchmark_runbook.md / docs/logic_overview.md / docs/checklists/p1-01.md を更新し、回帰テスト (`tests/test_run_daily_workflow.py`) で新フラグの伝播を確認。
...
- 2025-11-27: Reworked the yfinance fallback to use direct Yahoo Finance chart requests (no external package), rebuilt `validated/USDJPY/5m.csv` with 2018–2025 history, ingested live bars, and refreshed benchmarks via `run_benchmark_pipeline.py --disable-plot`. `scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6` now returns `ok: true` with ingest freshness ≈0.19 分 / benchmark age ≈0.07 時間。REST プロバイダ再開条件レビューは **Frozen**（実運用で従来経路が必要になった場合のみ再開）。
- 2025-11-28: Sandbox で `python3 scripts/run_daily_workflow.py --ingest --use-dukascopy --symbol USDJPY --mode conservative` を実行し、Dukascopy 未導入のまま yfinance 直呼び出しで 91 行を取得。`ops/runtime_snapshot.json.ingest_meta.USDJPY_5m.freshness_minutes=0.614` / `source_chain=["yfinance"]` を確認し、続けて `python3 scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6 --ingest-timeframe USDJPY_5m` が `ok: true`・`errors: []` を返すことを検証。チェックリスト / state.md / todo_next Archive を同期し、P1-04 の鮮度確認タスクをクローズ。
- 2025-12-03: `scripts/run_daily_workflow.py` の `--local-backup-csv` でチルダ展開を許可し、ホームディレクトリ配下の CSV を直接フォールバックに指定できるよう修正。`tests/test_run_daily_workflow.py::test_local_csv_fallback_expands_user_path` を追加し、メタデータの `local_backup_path` と fallback ログに絶対パスが保存されることを回帰確認。
- 2025-11-27: 環境はシングルPCの個人開発前提とし、`dukascopy-python` の社内パッケージ化／配布も **Frozen**。必要になった場合のみローカル `pip install dukascopy-python` で対応する。
- 2025-11-26: Normalised fallback stage names in `scripts/check_benchmark_freshness.py` so CLI output surfaces `ingest_meta.fallbacks` recorded by the workflow. Regression coverage updates confirm advisory downgrades still apply while exposing the recorded chain for review.
...
- 2025-10-04: Refactored `scripts/run_daily_workflow.py` ingestion dispatch to share context setup, provider runners, and command builders, keeping regression suite (`python3 -m pytest`) green while simplifying future provider additions.
...
- 2026-01-07: `scripts/run_daily_workflow.py` の `_build_pull_prices_cmd` がシンボル固有のローカル CSV を参照するよう修正し、`tests/test_run_daily_workflow.py::test_ingest_pull_prices_uses_symbol_specific_source` を追加してコマンド組み立てを回帰確認。
...
- **マルチ戦略比較バリデーション**: Day ORB と Mean Reversion (`strategies/mean_reversion.py`) を同一 CSV で走らせ、`docs/checklists/multi_strategy_validation.md` に沿ってゲート通過数・EV リジェクト数・期待値差をレビュー。DoD: チェックリストの全項目を完了し、比較サマリをレビュー用ドキュメントへ共有する。
