...
- 2025-11-19: `scripts/run_daily_workflow.py` に yfinance フェイルオーバーの鮮度ガードを追加し、取得データが閾値超過で陳腐化している際はローカル CSV + `synthetic_local` へ自動切替。`tests/test_run_daily_workflow.py` に回帰を追加し、`ingest_meta.fallbacks` に stale 理由が記録されることを確認。
