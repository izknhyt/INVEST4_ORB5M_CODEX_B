...
- 2026-10-30: Audited README / portfolio docs after the `runs/` cleanup, documented that Day ORB ships with `ev_mode=off` (`threshold_lcb_pip=-10.0`), and updated router snapshot guides to note that `runs/router_pipeline/latest` outputs are generated locally.
...
- 2026-06-12: Revalidated `--out-gap-*` exports against the then-available `validated/USDJPY/5m_with_header.csv`, documented default artefact locations and review steps in `docs/data_quality_ops.md`, and refreshed README samples to point at the headered snapshot used by the daily workflow. The headered file has since been removed from version control, so operators now rely on the headerless snapshot with the same commands.
...
- 2026-10-21: `scripts/update_state.py --simulate-live` の `diff.json` へ `reason[]`（`conditions_met` / `dry_run` / `override_disabled` / `anomaly:<type>`）を書き込み、`decision.status` / `decision.reasons` / `risk` を `--json-out` に含めて解析ログを補強。VAR 超過・オーバーライド停止・最大デルタ違反の pytest を `tests/test_update_state.py` へ追加し、`notifications/emit_signal.py` のフォールバック出力を `note=no_webhook_configured` + `env_var=SIGNAL_WEBHOOK_URLS` で明示化。関連運用手順を [docs/state_runbook.md](state_runbook.md#擬似ライブ更新フローscriptsupdate_statepy---simulate-live) / [docs/progress_phase4.md](progress_phase4.md) に同期。
...
- 2026-07-15: `data/usdjpy_5m_2018-2024_utc.csv` / `data/usdjpy_5m_2025.csv` / 既存短期スナップショットをマージし、`validated/USDJPY/5m.csv`（ヘッダ無し）と当時は併存していた `validated/USDJPY/5m_with_header.csv` を 2018–2025 通しに更新。短期ビューは `validated/USDJPY/5m_recent*.csv` へ退避し、`scripts/check_data_quality.py --calendar-day-summary` でギャップが週末由来であることを確認（coverage_ratio=0.71）。その後の runs/ クリーンアップ（2026-10-22）でヘッダ付きスナップショットは削除済みのため、長期ラン再実行時は共有リポジトリの `validated/USDJPY/5m.csv` を唯一のソースとして扱う。
...
- 2026-10-17: `configs/strategies/day_orb_5m.yaml` を `k_tp=1.25` / `k_sl=0.6` / `min_or_atr_ratio=0.16` / `rv_band_min_or_atr_ratio={low:0.18,mid:0.16,high:0.12}` / `ny_high_rv_min_or_atr_ratio=0.24` / `tokyo_low_rv_micro_trend_min=0.0` へ刷新し、`strategies/day_orb_5m.DayORB5m` の低 RV ロジックからセッション判定を排除。
  `_last_gate_reason` は `rv_filter=14215` / `tokyo_low_rv_guard=5608` から `or_filter=24` / `ny_high_rv_or_filter=1` へ大幅減少。
  再現: `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m.yaml --csv validated/USDJPY/5m.csv --mode conservative --out-dir runs/tmp/day_orb5m_debug --debug --debug-sample-limit 500000 --no-auto-state`
  → `python3 scripts/summarize_strategy_gate.py --run-dir runs/tmp/day_orb5m_debug/USDJPY_conservative_20251017_122719 --json`。
  ロングラン `runs/phase4/backtests/USDJPY_{conservative,bridge}_20251017_*` と差分 `reports/diffs/day_orb_reboot_metrics.json` を作成済み。
