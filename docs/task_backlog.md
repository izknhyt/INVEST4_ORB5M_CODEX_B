...
- 2025-11-20: `ops/runtime_snapshot.json.ingest_meta.USDJPY_5m` へフォールバックチェーンと合成バー情報を反映し、Sandbox で `python3 scripts/check_benchmark_freshness.py --target USDJPY:conservative --max-age-hours 6` が `ok=true` + `advisories` となることを確認。`docs/checklists/p1-04_api_ingest.md` / `docs/todo_next.md` に鮮度チェック結果と依存導入後の再実行タスクを追記。
