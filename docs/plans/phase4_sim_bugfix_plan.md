...
- **Dataset readiness**: `validated/USDJPY/5m.csv` currently spans **2018-01-01T00:00:00Z–2025-10-02T22:15:00Z** (latest bar reconfirmed via `tail -n 1`). W0 must record SHA256 + row count in `docs/progress_phase4.md` and mirror the numbers in `state.md` for reproducibility before any remediation starts.
- **Coverage extension backlog**: When upstream ingest delivers bars beyond 2025-10-02T22:15:00Z, follow the `state.md#next-task` checklist—stage the new CSV under `validated/USDJPY/5m.csv`, rerun data-quality audits, log hashes/end timestamp in both documents, and rerun W1 baseline commands before declaring the extension complete.
