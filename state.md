...
- 2026-07-22: Hardened `scripts/run_sim.py` CSV ingestion so symbol filters ignore case differences, aligned run artefact naming with manifest casing, and replaced `datetime.utcnow()` usage with UTC-aware timestamps to silence deprecation warnings. Added regression coverage via `tests/test_run_sim_cli.py::test_load_bars_csv_matches_symbol_case_insensitively` and executed `python3 -m pytest tests/test_run_sim_cli.py` to confirm the fix.
