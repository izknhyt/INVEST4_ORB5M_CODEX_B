...
- 2026-11-02: Phase4 dataset coverage anchors realigned across docs by correcting the long-run command horizon to 2025-10-02T22:15:00Z (`docs/plans/phase4_validation_plan.md`, `docs/progress_phase4.md`) and fixing the duplicated 2024 coverage note in `state.md`. Executed `python3 -m pytest` to confirm regressions remain green (418 passed / 5 skipped).
- 2026-11-02: Task: Maintain Phase4 dataset coverage anchors. DoD: 1) `docs/plans/phase4_validation_plan.md` のロングラン手順を 2025-10-02T22:15:00Z 終端に揃える、2) `docs/progress_phase4.md` / `state.md` から 2024-12-31 終端の記述を排除して最新フィンガープリント（579,578 行 / SHA256=85fa08f2224eb6119878f3689a5af617cb666eaab37c5acb7e3603c4bfda48d4 / tail=2025-10-02T22:15:00Z）へ統一する、3) `python3 -m pytest` で差分確認を行うこと。
- 2026-11-02: Documented the removal of the headered validated snapshot across docs by clarifying that only `validated/USDJPY/5m.csv` ships in-repo (updates: `docs/progress_phase4.md`, `docs/task_backlog.md`). Documentation-only change; tests not run.
- 2026-11-02: Task: Document the removal of the headered validated snapshot so operators rely on `validated/USDJPY/5m.csv`. DoD: README / `docs/data_quality_ops.md` / `docs/task_backlog.md` / `docs/progress_phase4.md` から `validated/USDJPY/5m_with_header.csv` を既定ソースとして扱う表現を更新し、現状はヘッダ無し CSV のみが共有リポジトリに含まれることを明示。ドキュメント更新のみのためテスト未実施。（2026-11-02 上記ログで完了報告済み）
...
- 2026-10-30: README と Day ORB / ポートフォリオ関連ドキュメントを runs/ クリーンアップ後の実態に合わせて再監査し、EV 無効化 (`ev_mode=off` / `threshold_lcb_pip=-10.0`) と router snapshot 出力がローカル生成である旨を追記。`docs/task_backlog.md` を同期し、`python3 -m pytest` を完走して 418 passed / 5 skipped を確認。
...
- 2026-10-28: Lowered the guard-relaxed RV-band ATR floors to `{high:0.08, mid:0.10, low:0.14}` while keeping
  `min_or_atr_ratio=0.16`, `max_loss_streak=4`, and `max_daily_loss_pips=180`. Validation steps:
  `python3 -m pytest tests/test_strategy_manifest.py`,
  `python3 -m pytest tests/test_runner.py -k guard_relaxed`,
  ...
  `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m_guard_relaxed.yaml --csv validated/USDJPY/5m.csv --symbol USDJPY --mode bridge --out-dir runs/phase4/backtests_guard_relaxed --no-auto-state --debug --debug-sample-limit 600000`,
  `python3 scripts/summarize_strategy_gate.py --run-dir runs/phase4/backtests_guard_relaxed/USDJPY_conservative_20251018_030339 --stage or_filter --json --out-json reports/diffs/conservative_guard_relaxed_strategy_gate_next.json`,
  `python3 scripts/summarize_strategy_gate.py --run-dir runs/phase4/backtests_guard_relaxed/USDJPY_bridge_20251018_030536 --stage or_filter --json --out-json reports/diffs/bridge_guard_relaxed_strategy_gate_next.json`,
  `python3 scripts/compare_metrics.py --left reports/diffs/conservative_guard_relaxed_metrics.json --right runs/phase4/backtests_guard_relaxed/USDJPY_conservative_20251018_030339/metrics.json --out-json reports/diffs/conservative_guard_relaxed_metrics_next.json`,
  `python3 scripts/compare_metrics.py --left reports/diffs/bridge_guard_relaxed_metrics.json --right runs/phase4/backtests_guard_relaxed/USDJPY_bridge_20251018_030536/metrics.json --out-json reports/diffs/bridge_guard_relaxed_metrics_next.json`,
  `python3 analysis/or_filter_guard_relaxed_summary.py --input conservative=reports/diffs/conservative_guard_relaxed_strategy_gate_next.json --input bridge=reports/diffs/bridge_guard_relaxed_strategy_gate_next.json --params-json runs/phase4/backtests_guard_relaxed/USDJPY_conservative_20251018_030339/params.json --base-drop 0.02 --floor 0.05 --json-output reports/diffs/or_filter_guard_relaxed_summary.json --markdown-output reports/diffs/or_filter_guard_relaxed_summary.md`.
  Outcomes: both modes still trade 8 times (Sharpe≈-5.02) while `or_filter` blocks fall to 171 (mid 99 / low 38 / high 34,
  `min_or_atr_ratio` mean ≈0.1049). Updated `analysis/or_filter_guard_relaxed_summary.py` to accept `stages.or_filter` payloads,
  regenerated `reports/diffs/or_filter_guard_relaxed_summary.(json|md)` / `reports/diffs/*_metrics_next.json`,
  and refreshed `docs/progress_phase4.md`, `docs/task_backlog.md#P4-01`, `docs/todo_next.md`.
...
- 2026-10-27: Sampled guard-relaxed behaviour on two short windows to evaluate loss/daily guard thresholds. Commands per mode: `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m_guard_relaxed.yaml --csv validated/USDJPY/5m.csv --symbol USDJPY --mode <mode> --start-ts 2025-06-01T00:00:00Z --end-ts 2025-07-01T00:00:00Z --out-dir runs/phase4/backtests_guard_relaxed --no-auto-state --debug --debug-sample-limit 200000` and `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m_guard_relaxed.yaml --csv validated/USDJPY/5m.csv --symbol USDJPY --mode <mode> --start-ts 2024-01-01T00:00:00Z --end-ts 2024-04-01T00:00:00Z --out-dir runs/phase4/backtests_guard_relaxed --no-auto-state --debug --debug-sample-limit 200000`. Verified `loss_streak_guard` / `daily_loss_guard` remained inactive with `python3 scripts/summarize_strategy_gate.py --run-dir runs/phase4/backtests_guard_relaxed/USDJPY_conservative_20251018_021103 --stage loss_streak_guard --stage daily_loss_guard --json` and the bridge variant. Estimated candidate thresholds via inline analysis scripts (e.g., `python3 - <<'PY'` blocks) to simulate `max_loss_streak` = 3〜5 and `max_daily_loss_pips` = 150〜220, confirming only `max_loss_streak=3` introduced additional (loss-avoiding) blocks. Synced findings to `docs/progress_phase4.md`, `docs/plans/phase4_sim_bugfix_plan.md`, `docs/todo_next.md`, and `docs/task_backlog.md`.
- 2026-10-25: Day ORB 最適化バンドルの自動化を統合。`scripts/run_daily_workflow.py --day-orb-optimization` でデータ監査→スイープ→Router構築→Paper判定→承認パケット生成を一括実行できるようにし、`configs/day_orb/optimization_bundle.yaml` / `configs/day_orb/paper_validation.yaml` と `scripts/generate_paper_validation.py` を追加。`tests/test_run_daily_workflow.py` / `tests/test_update_state.py` / `tests/test_generate_paper_validation.py` に回帰を追加し、`docs/go_nogo_checklist.md`・`docs/progress_phase4.md`・`ops/cron/day_orb_weekly.yaml`・`.github/workflows/day_orb_bundle.yml` を更新。実行: `python3 scripts/run_daily_workflow.py --day-orb-optimization --day-orb-config configs/day_orb/optimization_bundle.yaml --day-orb-dry-run --day-orb-output reports/day_orb/automation/day_orb_bundle_dry_run.json`, `python3 scripts/generate_paper_validation.py --config configs/day_orb/paper_validation.yaml --out reports/day_orb/paper_validation.json`, `python3 -m pytest tests/test_run_daily_workflow.py tests/test_update_state.py tests/test_generate_paper_validation.py`。
- 2026-10-25: P0-20〜P0-22 Closed。`docs/task_backlog.md` を完了表記へ更新し、進捗ドキュメントの未消化メモが無いことを確認。検証として `PYTHONPATH=. python3 scripts/recover_experiment_history.py --from-json --dry-run` を実行したが、PyArrow 非導入のため `error: PyArrow is required...` が表示されることを確認（環境未整備のためスキップ）。
...
- 2026-10-16: Ran the paper Go/No-Go audit dry-run: executed `python3 scripts/check_data_quality.py --csv validated/USDJPY/5m.csv --symbol USDJPY --out-json reports/data_quality/paper_gate.json --calendar-day-summary --fail-under-coverage 0.995` (coverage_ratio=0.7108 < 0.995), reran `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m.yaml --csv validated/USDJPY/5m.csv --mode conservative/bridge --out-dir runs/go_nogo/<mode> --out-json reports/go_nogo_<mode>.json --out-daily-csv reports/go_nogo_<mode>_daily.csv` (both modes Sharpe=-9.83 / MaxDD=-69.20), and launched `python3 scripts/run_daily_workflow.py --optimize --analyze-latency --archive-state --dry-run` which failed with `auto_optimize.py` JSONDecodeError after emitting optimisation runs (captured `reports/daily_workflow_dry_run.log`). Verified notifications via `PYTHONPATH=. python3 scripts/analyze_signal_latency.py --input ops/signal_latency.csv --out-json reports/latency_summary.json --rollup-output reports/latency_summary.csv` (p95=2100ms, failure_rate=0) and audited state retention with `python3 scripts/prune_state_archive.py --dry-run --keep 5` (63 candidate removals, conservative latest 5 recorded). Updated `docs/go_nogo_checklist.md` evidence columns and logged the No-Go judgement in `docs/progress_phase4.md#運用チェックリスト`.
- 2026-10-21: Replaced the pandas shim in `tests/test_ev_vs_actual_pnl.py` with a `pytest.importorskip("pandas")` guard so the module under test exercises real DataFrame behaviour instead of `_DummyFrame`, unblocking the failing pytest run. Executed `python3 -m pytest` to confirm the suite remains green.
- 2026-10-21: Hardened `scripts/update_state.py --simulate-live` by persisting `reason[]` alongside `status` in diff archives, surfacing `decision.status` / `decision.reasons` / `risk.*` in stdout JSON, and enriching rollback notifications with `log_fallback(..., extra={"env_var": "SIGNAL_WEBHOOK_URLS", ...})`. Added regression coverage for max-delta, VAR-cap, and override-disabled scenarios in `tests/test_update_state.py`, expanded `tests/test_emit_signal.py` to assert `note=no_webhook_configured` + metadata on missing webhook fallbacks, and synchronised the runbook/progress/backlog docs. Validation: `python3 scripts/update_state.py --simulate-live --dry-run --max-delta 0.2 --var-cap 0.04 --liquidity-cap 5.0 --json-out out/state_update_preview.json` (reported `no_new_bars` against the current snapshot) and `python3 -m pytest tests/test_update_state.py tests/test_emit_signal.py`.
- 2026-10-19: Introduced RV-band-specific `min_or_atr_ratio` floors for the guard-relaxed Day ORB manifest. Updated `core/runner.py`
  to propagate per-band thresholds into `EntryContext`, refreshed `strategies/day_orb_5m.DayORB5m` to consume runtime overrides,
  and extended `scripts/run_sim.py` snapshots so diff artefacts capture the new mapping. Re-ran
  `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m_guard_relaxed.yaml --csv validated/USDJPY/5m.csv --symbol USDJPY --mode conservative --out-dir runs/phase4/backtests_guard_relaxed --no-auto-state --debug --debug-sample-limit 600000`
  と `--mode bridge` で `runs/phase4/backtests_guard_relaxed/USDJPY_conservative_20251017_050450` / `USDJPY_bridge_20251017_050746`
  を取得し、`analysis/or_filter_guard_relaxed_summary.py --input conservative=reports/diffs/conservative_guard_relaxed_strategy_gate.json --input bridge=reports/diffs/bridge_guard_relaxed_strategy_gate.json --json-output reports/diffs/or_filter_guard_relaxed_summary.json --markdown-output reports/diffs/or_filter_guard_relaxed_summary.md`
  で `or_filter=278`（mid 137 / high 100 / low 41, `min_or_atr_ratio` 平均 ≈0.1387）へ差し替え。`docs/progress_phase4.md`・`docs/todo_next.md` を同期し、回帰として
  `python3 -m pytest tests/test_run_sim_cli.py tests/test_runner.py` と `python3 -m pytest tests/test_strategy_manifest.py` を完走。
...
- 2026-10-17: Brought the Day ORB parameter sweep loop online (P0-21). Added `configs/experiments/day_orb_core.yaml`, implemented
- 2026-10-17: Relaxed the Day ORB low-RV guard for P4-04. Updated `configs/strategies/day_orb_5m.yaml` with `min_or_atr_ratio=0.20` / `ny_high_rv_min_or_atr_ratio=0.30` / `tokyo_low_rv_micro_trend_min=0.10` and wired `runner_config.allow_low_rv=true`, added Tokyo-only low RV gating with directional `micro_trend` checks to `strategies/day_orb_5m.DayORB5m`, and synced the embedded manifest copy under `reports/portfolio_samples/router_demo/metrics/configs/strategies/day_orb_5m.yaml`. Ran `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m.yaml --csv validated/USDJPY/5m.csv --mode conservative --out-dir runs --debug --debug-sample-limit 500000 --no-auto-state`, summarised `runs/USDJPY_conservative_20251017_005817` via `python3 scripts/summarize_strategy_gate.py --run-dir runs/USDJPY_conservative_20251017_005817 --json`, and confirmed regressions with `python3 -m pytest`.

  `scripts/run_param_sweep.py` (grid/random search, constraint logging, seasonal metrics) and `scripts/select_best_params.py`
  (feasible ranking emitter), then executed `python3 scripts/run_param_sweep.py --experiment configs/experiments/day_orb_core.yaml --search grid --max-trials 2 --dry-run --out runs/sweeps/day_orb_core_smoke`
  と `python3 scripts/select_best_params.py --experiment configs/experiments/day_orb_core.yaml --runs-dir runs/sweeps/day_orb_core_smoke --top-k 3 --out reports/simulations/day_orb_core/best_params.json`
  で成果物を確認。`docs/progress_phase4.md` / `docs/task_backlog.md#p0-21-day-orb-optimisation-engine-bring-up-open` に指揮ログと追跡メモを反映し、
  `reports/simulations/day_orb_core/best_params.json` を初期化。回帰は `python3 -m pytest` で全件確認。
...
- 2026-07-30: Re-confirmed `validated/USDJPY/5m.csv` validated coverage ends at 2025-10-02T22:15:00Z, updated the `Next Task` gate to quarantine future bars beyond that timestamp, and aligned `docs/plans/phase4_sim_bugfix_plan.md` objectives/baseline commands with the 2018-2025 window. ドキュメントのみの更新のためテスト実行は不要と判断。
...
- 2026-07-30: Phase4 plan updated to reflect validated coverage through 2025-10-02T22:15:00Z and clarify that later bars remain backlog work. Synced `docs/plans/phase4_sim_bugfix_plan.md` with the Next Task checklist, refreshed state hash verification instructions (command now tails `2025-10-02T22:15:00Z`), and confirmed future extensions must update both `docs/progress_phase4.md` and this block in tandem.
...
## Next Task
- Maintain Phase4 dataset coverage for USDJPY 5m: the validated snapshot presently spans **2018-01-01T00:00:00Z–2025-10-02T22:15:00Z** (tail reconfirmed 2026-10-26 via `tail -n 1 validated/USDJPY/5m.csv`). Keep this section and [docs/plans/phase4_sim_bugfix_plan.md §0.2](docs/plans/phase4_sim_bugfix_plan.md#02-dependencies--interfaces) aligned. Latest audit (2026-10-26) preserved **579,578 rows / SHA256=85fa08f2224eb6119878f3689a5af617cb666eaab37c5acb7e3603c4bfda48d4 / end=2025-10-02T22:15:00Z**, with `coverage_ratio=0.7108490774310342` causing the guardrail command to exit non-zero as expected.
...
- Follow up on P4-01 guard tuning by expanding the short-window simulations (`max_loss_streak=3`, `max_daily_loss_pips=150` candidate) into sandbox reruns, comparing Bridge/Conservative metrics against the current guard-relaxed baseline, and packaging the diff (blocked trade counts, avoided pips, OR filter shifts) for review. Update `docs/progress_phase4.md#長期バックテスト` and the linked backlog/todo entries with the chosen follow-up before touching the long-run manifest.
