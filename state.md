...
- 2026-07-08: Restored Phase4 backtest command compatibility by teaching `scripts/run_sim.py` to accept `--out-json` / `--out-daily-csv` aliases and emit standalone daily CSVs. Extended CLI regression coverage in `tests/test_run_sim_cli.py` and synced the backlog note under [P4-01](docs/task_backlog.md#p4-01-長期バックテスト改善). Executed `python3 -m pytest` (347 tests) to validate the suite.
...
## Next Task
- Execute the Phase 4 validation plan kickoff: create `runs/phase4/backtests/` parameter sweep directories and capture the baseline Conservative/Bridge long-run metrics via `python3 scripts/run_sim.py --manifest configs/strategies/day_orb_5m.yaml --csv validated/USDJPY/5m.csv --mode <mode> --start-ts 2018-01-01T00:00:00Z --end-ts 2025-12-31T23:55:00Z --out-json reports/long_<mode>.json --out-daily-csv reports/long_<mode>_daily.csv`. Publish the refreshed metrics/command list in `docs/progress_phase4.md` and record artefact paths for follow-up experiments。
