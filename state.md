...
- [P0-10] 2025-09-28: Updated `scripts/aggregate_ev.py` to register the repository root on `sys.path`, added a regression test that writes minimal state archives and verifies YAML/CSV outputs, and confirmed `python3 -m pytest tests/test_aggregate_ev.py` and `python3 scripts/run_sim.py --csv data/usdjpy_5m_2018-2024_utc.csv --symbol USDJPY --mode conservative --equity 100000 --json-out /tmp/sim.json` complete successfully.
