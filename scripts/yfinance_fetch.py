...
def fetch_bars(
    ...
    *,
    ...
    fetch_start = start_naive
    if interval.endswith("m"):
        fetch_start = max(end_clamped - timedelta(days=7), start_naive)

    payload = _download_chart_json(ticker, interval, fetch_start, end_clamped)
    return _iter_quote_rows(
        payload,
        symbol=symbol,
        tf=tf,
        start=start_naive,
